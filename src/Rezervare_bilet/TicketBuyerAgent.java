	    ...
	    public void action() {
	    	...
	    	case 0:
	    		...
	    		mt = MessageTemplate.and(MessageTemplate.MatchConversationId(
	    				"Ticket-trade"), MessageTemplate.MatchInReplyTo(cfp.
	    				getReplyWith()));
	    		step = 1;
	    		break;
	    	...
	    	case 2:
	    		...
	    		mt = MessageTemplate.and(MessageTemplate.MatchConversationId(
	    			"Ticket-trade"), MessageTemplate.MatchInReplyTo(order_accept.
	    			getReplyWith()));
	    		step = 3;
	    		break;
	    	//se primeste informatie de la seller daca s-a cumparat cu succes sau nu
	    	case 3:
	    		reply = myAgent.receive(mt);
	    		if (reply != null) {
	    			if (reply.getPerformative() == ACLMessage.INFORM) {
	    				myGui.receiveResult(reply.getSender().getLocalName(), 
	    						"SUCCES");
	    			} else {
	    				myGui.receiveResult(reply.getSender().getLocalName(), 
	    						"FAILURE");
